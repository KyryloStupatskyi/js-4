...
let wordsList = function(str, subStr) {
  ...
  let res = str
    ...
    .toLowerCase()
    .split(' ')
    .filter((strItem) => strItem.indexOf(subStr) > -1);
...
const reg = new RegExp(':\\d{2}$', 'gui'); // flags g - глобальный поиск u - символы UNICODE i - регистронезависимость \d символьные значения $ - конец ввода
...
let getDateAgo = (date, days) => {
  ...
  return dateX.toLocaleString().replace(/(\d.*),\s+(\d.*)/gu, '$1');
};
...
Object.defineProperties(Car.prototype, {
  used: {
    get() {
      ...
      return year - this.year > 2 ? 'used' : 'new';
    },
